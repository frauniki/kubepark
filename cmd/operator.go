...
func init() {
	...
	utilruntime.Must(kubeparkdevv1alpha1.AddToScheme(scheme))
	// +kubebuilder:scaffold:scheme
}

// newOperatorCommand wraps the kubebuilder-scaffolded manager entrypoint.
// Flag parsing is delegated to the standard flag package so that the
// scaffolded flags (and zap's) keep working unchanged.
func newOperatorCommand() *cobra.Command {
	return &cobra.Command{
		Use:                "operator",
		Short:              "Run the kubepark controller manager",
...
func runOperator(args []string) {
	...
	mgr, err := ctrl.NewManager(ctrl.GetConfigOrDie(), ctrl.Options{
		...
		LeaderElectionID:       "de7f76eb.kubepark.dev",
	...
	if err := (&controller.SandboxSessionReconciler{
		Client: mgr.GetClient(),
		Scheme: mgr.GetScheme(),
	}).SetupWithManager(mgr); err != nil {
		setupLog.Error(err, "Failed to create controller", "controller", "sandboxsession")
		os.Exit(1)
	}
